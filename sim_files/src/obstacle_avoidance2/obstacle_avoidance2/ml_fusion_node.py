...
class MlFusionNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(f"==== UWAGA: NASŁUCHUJĘ KAMERY NA: {image_topic} ====")
    ...
    def image_callback(self, msg):
        self.get_logger().info("image dziala")
        ...
        self.x_factor = orig_w/IMG_SIZE
        self.y_factor = orig_h/IMG_SIZE
        ...
        for i in valid_indices:
            ...
            scaled_x = x*self.x_factor
            scaled_y = y*self.y_factor
            scaled_w = w*self.x_factor
            scaled_h = h*self.y_factor

            boxes.append([int(scaled_x - scaled_w/2), int(scaled_y - scaled_h/2), int(scaled_w), int(scaled_h)])
        ...
        if len(indices) > 0:
            for i in indices.flatten():
                ...
                color = (0, 0, 255) if name in ('pedestrian', 'car', 'motorcycle') else \
                        (0, 165, 255) if name in ('pothole', 'bad road', 'obstacle') else (0, 255, 0)
