...
class SafetyGuardNode(Node):
    def __init__(self):
        ...
        self.state = "NORMAL" #"AVOID_TURN", "AVOID_FORWARD"
    ...
    def get_scan_slice(self, msg, start_angle_deg, end_angle_deg):
        ...
        slice_ranges = msg.ranges[start_idx:end_idx]
        return [r for r in slice_ranges if 0.05< r < 10.0]

    def scan_callback(self,msg):
        self.latest_scan = msg
        front_slice = self.get_scan_slice(msg,-30,30)
        min_front_dist = min(front_slice) if front_slice else float('inf')

        self.lidar_clear = min_front_dist > self.SAFE_DISTANCE

        if self.state == "NORMAL" and not self.lidar_clear:
            self.get_logger().warn(f"LIDAR WYKRYŁ PRZESZKODĘ! ({min_front_dist:.2f}m). Omijam!")
    ...
    def vision_callback(self, msg):
        ...
        if self.state == "NORMAL" and self.camera_clear == False:
                self.get_logger().warn(f"KAMERA WYKRYŁA ZAGROŻENIE")
                self.trigger_avoidance()                
    ...
    def trigger_avoidance(self):
        if self.latest_scan:
            ...
            right_slice = self.get_scan_slice(self.latest_scan, -30, -80)
        ...
        self.state_start_time = time.time() # ZAPISUJEMY CZAS STARTU MANEWRU
        self.publish_stop()
    ...
    def state_machine(self):
        ...
        if self.state == "AVOID_TURN":
            if not self.clear_path_ahead:
                ...
                self.state_start_time = now # Reset stopera dla jazdy prosto
    ...
    def is_side_clear(self):
        ...
        if self.current_turn_direction > 0:
            side_slice = self.get_scan_slice(self.latest_scan, -100, -30) # Patrzymy w prawo
        else:
            side_slice = self.get_scan_slice(self.latest_scan, 30, 100)   # Patrzymy w lewo
