...
class ZenohHandler(QObject):
    ...
    def _connect_worker(self):
        try:
            ...
            self._publishers["mowing_status"] = self.session_tcp.declare_publisher("rt/mowing_status")
    ...
    def send_mowing(self):
        self._robot_mowing = True
        payload = struct.pack('<4s?', b'\x00\x01\x00\x00', True)
        self._publish("mowing_status", payload)
        self.pqt_sig_status.emit("mowing on")

    def send_Notmowing(self):
        self._robot_mowing = False
        payload = struct.pack('<4s?', b'\x00\x01\x00\x00', False)
        self._publish("mowing_status", payload)
        self.pqt_sig_status.emit("mowing off")
