...
from config import SERVER_IP, PORT, STOP_INTERVAL, MAN_CMD_VEL_INTERVAL, CMD_VEL_SPEED, CMD_VEL_LIN_STEP, CMD_VEL_ANG_STEP
import os, sys
...
class MowingButton(QPushButton):
    def __init__(self, text, parent=None):
        super().__init__(text, parent)
        self.setMinimumSize(200, 200)
        self.setMaximumSize(600, 600)
        ...
        self.offColor = "#1a076880"
        self.onColor = "#350ae2ff"
        self._update_style(600)
    ...
    def toggle_mow(self):
        ...
        if self._stopped:
            self.setText("OFF")
            self._update_style_color(size, self.offColor, self.onColor) #off, on
        else:
            self.setText("ON")
            self._update_style_color(size, self.onColor, self.offColor) #on, off

    def _update_style(self, size):
        self._update_style_color(size, self.offColor, self.onColor)
    ...
    def resizeEvent(self, event):
        size = min(self.width(), self.height())
        color = self.offColor if self._stopped else self.onColor
        pressed = self.onColor if self._stopped else self.offColor
        self._update_style_color(size, color, pressed)
...
class SteeringWidget(QFrame):
    def __init__(self, zenoh_handler, parent=None):
        ...
        self._lin_step = CMD_VEL_LIN_STEP
        self._ang_step = CMD_VEL_ANG_STEP
    ...
    def _build_ui(self):
        ...
        self._btn_forward  = self._make_arrow_btn("▲", "forward")
        self._btn_backward = self._make_arrow_btn("▼", "backward")
        self._btn_left     = self._make_arrow_btn("◄", "left")
        self._btn_right    = self._make_arrow_btn("►", "right")
    ...
    def _make_arrow_btn(self, text, state_key):
        ...
        btn.pressed.connect(lambda sk=state_key: self._set_direction_state(sk, True))
        btn.released.connect(lambda sk=state_key: self._set_direction_state(sk, False))
        return btn
    ...
    def _set_direction_state(self, state_key, active: bool):
        if not self._manual:
            return
        self._drive_states[state_key] = active
        self._update_targets()

    def _update_targets(self):
        if self._drive_states["forward"]:
            self._target_linear = CMD_VEL_SPEED
        elif self._drive_states["backward"]:
            self._target_linear = -CMD_VEL_SPEED
        else:
            self._target_linear = 0.0

        if self._drive_states["left"]:
            self._target_angular = CMD_VEL_SPEED  
        elif self._drive_states["right"]:
            self._target_angular = -CMD_VEL_SPEED
        else:
            self._target_angular = 0.0

        if (self._target_linear != 0.0 or self._target_angular != 0.0 or 
                self._linear != 0.0 or self._angular != 0.0):
            if not self._cmd_timer.isActive():
                self._cmd_timer.start()

    def _send_current_cmd(self):
        #linear
        if self._linear < self._target_linear:
            self._linear = min(self._linear + self._lin_step, self._target_linear)
        elif self._linear > self._target_linear:
            self._linear = max(self._linear - self._lin_step, self._target_linear)

        #angular
        if self._angular < self._target_angular:
            self._angular = min(self._angular + self._ang_step, self._target_angular)
        elif self._angular > self._target_angular:
            self._angular = max(self._angular - self._ang_step, self._target_angular)

        #send
        self.zenoh.send_cmd_vel(self._linear, self._angular)

        #stop timer
        if (self._target_linear == 0.0 and self._target_angular == 0.0 and 
                self._linear == 0.0 and self._angular == 0.0):
    ...
    def _set_mode(self, mode: str):
        ...
        if not self._manual:
            self._cmd_timer.stop()
            for key in self._drive_states:
                self._drive_states[key] = False
            self._linear = 0.0
            self._angular = 0.0
            self._target_linear = 0.0
            self._target_angular = 0.0
    ...
    def key_press(self, key):
        if not self._manual:
            return
        mapping = {
            Qt.Key.Key_W: ("forward", self._btn_forward),
            Qt.Key.Key_S: ("backward", self._btn_backward),
            Qt.Key.Key_A: ("left", self._btn_left),
            Qt.Key.Key_D: ("right", self._btn_right),
        }
        if key in mapping:
            state_key, btn = mapping[key]
            self._highlight_btn(btn, True) 
            self._set_direction_state(state_key, True)
    ...
    def key_release(self, key):
        ...
        if key in mapping:
            state_key, btn = mapping[key]
            self._highlight_btn(btn, False)  
            self._set_direction_state(state_key, False)
...
class MapApp(QWidget):
    ...
    def update_robot_marker(self, lat, lon):
        if hasattr(self, "robot_marker"): #remove old robot position
            self.map.removeLayer(self.robot_marker)

        self.robot_marker = L.marker([lat, lon]) #mark new position
        self.robot_marker.bindTooltip("Chwast")
        self.robot_marker.addTo(self.map)

        if not hasattr(self, "map_centered_on_robot"): #center on robot once
...
class Dashboard(QMainWindow):
    ...
    def organise_UI(self):
        def _organise_main_window():
            ...
            #mowing panel
            self.mowing_widget = QWidget()
            mowing_layout = QVBoxLayout(self.mowing_widget)
            self.mowing_button = MowingButton("Mowing")
            ...
            self.mowing_button.clicked.connect(self.send_mowing)
            mowing_layout.addWidget(self.mowing_button, alignment=Qt.AlignmentFlag.AlignCenter)
            ...
            self.mowing_widget.setStyleSheet("border: 2px solid #89b4fa;")
            ...
            top_splitter.addWidget(self.mowing_widget)
    ...
    def _on_frame(self, frame: np.ndarray):
        if not self.zenoh.is_connected():  
            return
        h, w, ch = frame.shape
        qt_img = QImage(frame.data, w, h, ch * w, QImage.Format.Format_BGR888).copy()
    ...
    def send_mowing(self):
        self.mowing_button.toggle_mow()
        if  self.mowing_button._stopped:
            self.zenoh.send_Notmowing()       
            #print("F")
        else:
            self.zenoh.send_mowing()
            #print("T")
