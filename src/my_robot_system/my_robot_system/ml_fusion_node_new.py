...
def draw_label(frame, text, x1, y1, color):
    ...
    # Jeśli napis nie mieści się nad boxem, rysuj wewnątrz
    if y1 - th - 6 < 0:
        ty = y1 + th + 4
    else:
        ty = y1 - 5
    ...
    cv2.putText(frame, text, (x1 + 2, ty), font, scale, color, thickness)
class TensorRTInfer:
...
class MlFusionNode(Node):
    ...
    def image_callback(self, msg):
        ...
        for i in valid_indices:
            ...
            scores.append(min(float(scores_all[i]) + 0.1, 1.0))
        ...
        if len(indices) > 0:
            for i in indices.flatten():
                ...
                draw_label(frame, f"{name}: {conf:.2f}{distance_str}", x1, y1, color)
