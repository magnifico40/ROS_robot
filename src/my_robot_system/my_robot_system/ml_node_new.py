    ...
    def image_callback(self, msg):
        ...
        if len(indices) > 0:
            for i in indices.flatten():
                ...
                if name == 'person':
                    color = (0, 0, 255)  # Czerwony (Format BGR)
                elif name == 'obstacle':
                    color = (0, 165, 255)  # Pomarańczowy
                elif name == 'grass':
                    color = (0, 255, 0)  # Zielony
                else:
                    color = (255, 255, 255)  # Biały jako zapasowy
        ...
        self.prev_time = curr
        cv2.putText(frame, f"FPS:{fps:.1f} Infer:{infer_ms:.0f}ms", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                    (0, 255, 255), 2)
